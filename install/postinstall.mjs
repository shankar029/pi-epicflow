...
// v0.12.0: pick the platform-appropriate scripts directory. POSIX gets
// bash scripts symlinked into ~/.local/bin; Windows gets .ps1 files plus
// generated .cmd shims (symlinks need admin / Developer Mode on Windows).
const SCRIPTS_SRC = IS_WIN
  ? path.join(PKG_ROOT, "skills", "epic-feature-workflow", "scripts-win")
  : path.join(PKG_ROOT, "skills", "epic-feature-workflow", "scripts");
...
// v0.12.0 — Windows shim installer. Writes a `pi-<name>.cmd` into BIN_DST
// that exec's the .ps1 with -ExecutionPolicy Bypass so it works even on
// locked-down corp boxes whose system policy is Restricted. Idempotent:
// only overwrites a .cmd that looks like one of ours (contains the
// pi-epicflow marker line).
const WIN_CMD_MARKER = "@REM pi-epicflow-shim v1";
function installWinShimSafely(psScriptAbs, binDir) {
  const base = path.basename(psScriptAbs, ".ps1"); // pi-epic-init
  const cmdDst = path.join(binDir, `${base}.cmd`);
  // PowerShell needs forward-or-backslash; use the OS path verbatim.
  // %* forwards every arg, quoted by cmd.exe's own rules.
  const body =
    `@echo off\r\n` +
    `${WIN_CMD_MARKER}\r\n` +
    `@REM Auto-generated by pi-epicflow postinstall. Do not edit by hand.\r\n` +
    `@REM Source: ${psScriptAbs}\r\n` +
    `powershell.exe -NoProfile -ExecutionPolicy Bypass -File "${psScriptAbs}" %*\r\n` +
    `exit /b %ERRORLEVEL%\r\n`;
  try {
    const existing = safeRead(cmdDst);
    if (existing) {
      if (existing.toString().includes(WIN_CMD_MARKER)) {
        // ours — safe to overwrite
        fs.writeFileSync(cmdDst, body);
        return;
      }
      warn(`${cmdDst} exists but isn't ours. Skipping — remove manually if you want pi-epicflow to manage it.`);
      return;
    }
    fs.writeFileSync(cmdDst, body);
    out(`installed shim: ${cmdDst} -> ${psScriptAbs}`);
  } catch (e) {
    warn(`could not install Windows shim for ${base}: ${e.message}`);
  }
}
...
// ── Step 2: bin shims ──────────────────────────────────────────────────────
// On POSIX: symlink each pi-* script into BIN_DST.
// On Windows: install pi-<name>.cmd shim that exec's the .ps1 with Bypass.
out(`installing pi-* ${IS_WIN ? "shims" : "symlinks"} into ${BIN_DST}`);
if (ensureDir(BIN_DST)) {
  let entries = [];
  try { entries = fs.readdirSync(SCRIPTS_SRC); }
  catch (e) { warn(`could not read ${SCRIPTS_SRC}: ${e.message}`); }
  const cliScripts = IS_WIN
    ? entries.filter(n => n.startsWith("pi-") && n.endsWith(".ps1"))
    : entries.filter(n => n.startsWith("pi-"));
  if (cliScripts.length === 0) {
    warn(`no pi-* scripts found under ${SCRIPTS_SRC}`);
  }
  for (const n of cliScripts) {
    const src = path.join(SCRIPTS_SRC, n);
    if (IS_WIN) {
      installWinShimSafely(src, BIN_DST);
    } else {
      symlinkScriptSafely(src, BIN_DST);
    }
  }
